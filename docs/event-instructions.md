# Rise of Kingdoms Event Instructions

This document is the maintained reference for Conclave event descriptions and daily-mail summaries. It covers the actions members should take, leadership considerations, reset rules, and the source quality behind each entry.

**Last reviewed:** August 3, 2026

**Time standard:** UTC

**Accuracy rule:** Current in-game event instructions and kingdom leadership directions always take priority over this reference.

## Daily-mail writing standard

Daily-mail summaries should:

- Lead with the action members need to take.
- Include important targets, eligibility requirements, and reset deadlines.
- Remain short enough to display completely in an in-game mail.
- Avoid dates for active events; Conclave adds the remaining-time label separately.
- Use `1 DAY ONLY` only when the event's complete duration is one day.
- Use `1 DAY LEFT` on the final day of a multi-day event.
- Display the event name and remaining-time label at size 23.
- Keep the action summary at the game's default text size so it remains visually distinct and readable.
- Avoid strategies that have not been verified in-game or by a reliable current source.

## Single-event announcement standard

The standalone announcements below are ready to paste into Conclave's in-game mail editor. Replace text inside square brackets before sending.

- Use `#855400` for the large event title.
- Use `#176B3A` for major action headings.
- Use `#1E5F8A` for timing and supporting information.
- Use `#963F00` for warnings and deadlines.
- Keep the title at size 34 and major headings at size 27 for readability.
- Confirm that the finished mail remains below the in-game 2,000-character limit.

---

## King of the Tribes

### Member instructions

- Be in an alliance before participating.
- Start or join alliance rallies against Barbarian Forts.
- Fill rallies with as many alliance members as possible. More participating members increase the alliance event points earned from a successful rally.
- Attack the highest fort level the rally can defeat reliably.
- Continue running and filling rallies to help the alliance reach its milestone and ranking goals.
- Check in-game mail for alliance milestone and final ranking rewards.

### Leadership considerations

- Coordinate frequent Barbarian Fort rallies throughout the event.
- Favor full, successful rallies over risky attempts that may fail.
- Publish any alliance-specific rally limits, fort levels, or participation rules.
- Confirm the current scoring rules from the in-game event page when the event opens.

### Daily-mail summary

> Join alliance rallies against Barbarian Forts. Fill every rally with as many members as possible and defeat the highest reliable fort level to increase our alliance score.

### Single-event announcement

**Mail subject:** `KING OF THE TRIBES`

```text
<size=34><b><color=#855400>KING OF THE TRIBES</color></b></size>
<color=#1E5F8A>[EVENT WINDOW — UTC]</color>

<size=27><b><color=#176B3A>RALLY THE ALLIANCE</color></b></size>
Start or join alliance rallies against Barbarian Forts. Fill every rally with as many alliance members as possible—more participants increase the points earned for our alliance.

Attack the highest fort level your rally can defeat reliably. Keep rallies moving throughout the event so we can reach every alliance milestone and improve our final ranking.

<b><color=#963F00>IMPORTANT</color></b>
You must be in the alliance to participate. Follow any rally limits or fort-level instructions issued by alliance leadership.
```

### Source notes

- A January 2025 player discussion confirms that active alliances continue to maximize the number of members participating in each rally.
- The detailed scoring and reward description comes from an older community guide and must be checked against the current in-game rules.
- Confidence: **Moderate**.

Sources:

- [Recent player confirmation](https://www.reddit.com/r/RiseofKingdoms/comments/1hx9ctn/)
- [Detailed event rules](https://ddfood.tistory.com/entry/%EB%9D%BC%EC%9D%B4%EC%A6%88%EC%98%A4%EB%B8%8C%ED%82%B9%EB%8D%A4%EC%A6%88-%EB%B6%80%EB%9D%BD%EC%9D%98-%EC%99%95-%EC%9D%B4%EB%B2%A4%ED%8A%B8)

---

## Mercantile Melee

### Member instructions

- Configure and dispatch your Personal Caravan with the strongest defensive lineup available.
- Complete all available Personal Caravan departures and collect the rewards delivered after successful arrivals.
- Use your available raid attempts against rival caravans.
- Review the target caravan's defending lineup and rewards before raiding, then choose a target your march can defeat.
- Remain in the alliance for at least 8 hours before applying to join the Alliance Caravan.
- Apply during the event's preparation phase.
- Review and configure the lineup requested by the event interface.
- Follow the Alliance Caravan Captain's formation and participation instructions.
- Remain available through the caravan's departure when selected.
- Personal Caravan rewards are delivered automatically through in-game mail after the caravan reaches its destination.

### Leadership considerations

- The Alliance Caravan Captain must have been in the alliance for at least 24 hours.
- Appoint the captain early enough to configure the defensive lineup before preparation ends.
- The captain must remain in the alliance and in the Lost Kingdom during preparation. Otherwise, the caravan can be canceled and its departure chance returned, subject to the daily cap.
- Review applications and communicate the final lineup before departure.

### Daily-mail summary

> Dispatch your Personal Caravan with a strong defense, then use your raid attempts against rival caravans you can defeat. Apply to the Alliance Caravan during preparation and follow the Caravan Captain's lineup instructions.

### Single-event announcement

**Mail subject:** `MERCANTILE MELEE`

```text
<size=34><b><color=#855400>MERCANTILE MELEE</color></b></size>
<color=#1E5F8A>[PREPARATION AND DEPARTURE TIMES — UTC]</color>

<size=27><b><color=#176B3A>TRADE AND RAID</color></b></size>
Dispatch your Personal Caravan with the strongest defensive lineup available. Complete your individual caravan opportunities and collect the rewards delivered after each successful arrival.

Use your available raid attempts against rival caravans. Review the defending lineup and reward first, then choose targets your march can defeat.

<size=27><b><color=#176B3A>ALLIANCE CARAVAN</color></b></size>
Apply during preparation and follow the Caravan Captain's lineup instructions. Governors need 8 hours in the alliance to apply; the captain requires 24 hours.

<b><color=#963F00>COMPLETE EVERY OPPORTUNITY</color></b>
Do not leave Personal Caravan departures or raid attempts unused. Finish your setup before Alliance Caravan preparation ends.
```

### Source notes

- The alliance-tenure requirements and caravan cancellation rules come from the official December 2025 update.
- Confidence: **High** for eligibility and preparation requirements; **moderate** for strategy recommendations.

Sources:

- [Official Winter's Embrace patch notes](https://forumshare-global.farlightgames.com/rok/post/2179486/?lang=en)
- [May 2025 strategy guide](https://www.youtube.com/watch?v=sivOPaO7cLk)

---

## Ceroli Crisis

### Member instructions

- Form a four-governor team and agree on the difficulty before entering.
- Select Tank, Damage, or Support skills that match your commander pair and the team's needs.
- Review the selected chieftain's strategy panel before starting because every boss has different mechanics.
- Spread out when the boss punishes grouped marches.
- Move out of marked danger areas and reposition when the encounter calls for it.
- Defeat spawned enemies when required instead of attacking only the boss.
- Coordinate taunts, healing, cleansing, and damage-boost skills with the team.
- Complete the available reward attempts before the event closes.

### Leadership considerations

- Help members build balanced teams rather than stacking only damage roles.
- Encourage players to select a difficulty the complete team can clear reliably.
- Share boss-specific instructions when the active chieftain requires special positioning or troop types.

### Daily-mail summary

> Form a four-governor team, choose your difficulty, role, and skills, then follow the selected boss's mechanics. Spread out, avoid marked attacks, handle spawned enemies, and use team skills at the right time.

### Single-event announcement

**Mail subject:** `CEROLI CRISIS`

```text
<size=34><b><color=#855400>CEROLI CRISIS</color></b></size>
<color=#1E5F8A>[EVENT WINDOW — UTC]</color>

<size=27><b><color=#176B3A>BUILD YOUR TEAM</color></b></size>
Form a four-governor team and agree on the difficulty before entering. Choose Tank, Damage, or Support skills that match your commander pair and the needs of the group.

<size=27><b><color=#176B3A>FOLLOW THE BOSS MECHANICS</color></b></size>
Spread out when attacks punish grouped marches, move out of marked danger areas, defeat spawned enemies when required, and save healing, taunts, cleanses, and damage boosts for the correct mechanic.

<b><color=#963F00>TEAMWORK WINS</color></b>
Review the selected chieftain's strategy panel before starting and coordinate movement and skills with the team.
```

### Source notes

- The January 2026 guide documents the team roles, active skills, positioning, spawned enemies, and boss-specific mechanics.
- Confidence: **High** for the general team instructions; individual boss mechanics should be confirmed in the current strategy panel.

Source:

- [January 2026 Ceroli Crisis guide](https://riseofkingdomsguides.com/rise-of-kingdoms-ceroli-crisis-guide/)

---

## Mighty Army

### Member instructions

Complete both groups of quests every day:

- Train 4,000 units.
- Continue to 6,000 trained units.
- Finish at 10,000 trained units.
- Defeat 5 Barbarians.
- Continue to 10 and then 15 defeated Barbarians.
- Finish at 30 defeated Barbarians.
- Claim every completed milestone before 00:00 UTC.

Progress resets at 00:00 UTC. Unfinished daily progress does not carry forward.

### Leadership considerations

- Remind members before reset to claim completed rewards.
- Coordinate Mighty Army training with other troop-training events when possible.
- Avoid recommending unnecessary speedup use unless leadership has identified a valuable overlap.

### Daily-mail summary

> Train 10,000 troops and defeat 30 Barbarians today. Claim every completed milestone before the daily reset at 00:00 UTC.

### Single-event announcement

**Mail subject:** `MIGHTY ARMY`

```text
<size=34><b><color=#855400>MIGHTY ARMY</color></b></size>
<color=#1E5F8A>[EVENT WINDOW — UTC]</color>

<size=27><b><color=#176B3A>DAILY OBJECTIVES</color></b></size>
Train 10,000 troops and defeat 30 Barbarians every day. Rewards unlock at the intermediate training and Barbarian milestones, so claim each completed objective as you progress.

Training milestones: 4,000 • 6,000 • 10,000
Barbarian milestones: 5 • 10 • 15 • 30

<b><color=#963F00>CLAIM BEFORE RESET</color></b>
Progress resets daily at 00:00 UTC. Claim all completed rewards before reset because unfinished daily progress does not carry forward.
```

### Source notes

- The current community event reference was recrawled in July 2026 and lists the daily milestones and reset rule.
- Confidence: **High**, subject to the current in-game event page.

Source:

- [Mighty Army event reference](https://riseofkingdoms.fandom.com/wiki/Events/Mighty_Army)

---

## Dark Fortress Raid

### Member instructions

- Note the raid time announced by alliance leadership.
- Be online and ready before the scheduled start whenever possible.
- Join alliance rallies attacking the Dark Fortress.
- Follow the assigned rally leaders, troop types, and march instructions.
- Continue filling rallies until the raid ends to maximize alliance participation and rewards.
- If the event offers offline participation, enable it before leaving the game. The feature can automatically add eligible offline governors to nearby Dark Fortress rallies during the battle or within 15 minutes before it begins.

### Leadership considerations

- Place the Raid Fortress and select the alliance's battle schedule.
- Announce the start time, rally leaders, troop recommendations, and any march restrictions.
- Encourage members who cannot remain online to enable offline participation.
- Confirm the reward mechanics displayed for the kingdom's current season because rewards can vary by progression.

### Daily-mail summary

> Join the scheduled alliance rallies against the Dark Fortress and follow the rally leaders' troop instructions. Enable offline participation when available so your troops can join nearby rallies automatically.

### Single-event announcement

**Mail subject:** `DARK FORTRESS RAID`

```text
<size=34><b><color=#855400>DARK FORTRESS RAID</color></b></size>
<color=#1E5F8A>[RAID DATE AND TIME — UTC]</color>

<size=27><b><color=#176B3A>ASSEMBLE FOR THE RAID</color></b></size>
Be ready before the scheduled start and join alliance rallies attacking the Dark Fortress. Keep filling rallies until the raid ends to maximize participation and rewards.

Rally leaders: [NAMES]
Troop instructions: [TROOP TYPES OR LIMITS]

<b><color=#963F00>CAN'T STAY ONLINE?</color></b>
Enable offline participation when the option is available. Eligible offline governors can automatically join nearby Dark Fortress rallies during the battle or within 15 minutes before it begins.
```

### Source notes

- Lilith's recent official announcement confirms the rally-based gameplay and offline-participation feature.
- Recent 2025 and 2026 guides cover placement, setup, and reward preparation.
- Confidence: **High** for rally and offline-participation instructions; **moderate** for season-dependent setup details.

Sources:

- [Official Rise of Kingdoms announcement](https://forum-global.lilithgame.com/post/2197960)
- [February 2026 early-game guide](https://www.youtube.com/watch?v=HIefNScjvF8)
- [August 2025 placement and reward guide](https://www.youtube.com/watch?v=kSgBXdi3F5w)

---

## The Golden Kingdom

### Member instructions

- Select five armies carefully before entering. Commanders, talents, and equipment cannot be changed after the event begins.
- Use durable front-line armies supported by strong area-damage and support commanders.
- Clear fog to find useful items and the Guardian Chief, but avoid unnecessary fights because army health persists between battles.
- Destroy Arrow Towers promptly because they damage your armies as additional fog is cleared.
- Defeat the Guardian Chief to advance to the next floor.
- Preserve healing, powerful relics, and Karaku Gold for the more difficult later floors.
- Collect checkpoint rewards after every fourth completed floor.
- Remember that Army Expansion does not apply; other eligible account, equipment, and troop buffs do.

Damage suffered inside the event does not reduce troop power outside the event.

### Leadership considerations

- Encourage members to complete the event for its individual rewards.
- Avoid prescribing one fixed commander lineup because the best five armies depend on account age and available commanders.
- Note that event floor count or encounters can vary when Lilith tests an updated version. The in-game rules are authoritative.

### Daily-mail summary

> Choose five strong armies, clear the fog, and defeat each Guardian Chief. Avoid unnecessary fights, protect your army health, and save healing and powerful relics for the later floors.

### Single-event announcement

**Mail subject:** `THE GOLDEN KINGDOM`

```text
<size=34><b><color=#855400>THE GOLDEN KINGDOM</color></b></size>
<color=#1E5F8A>[EVENT WINDOW — UTC]</color>

<size=27><b><color=#176B3A>ENTER PREPARED</color></b></size>
Choose five strong armies before entering. Your commanders, talents, and equipment cannot be changed after the event begins, so include durable front-line armies plus strong area-damage and support commanders.

Clear the fog, destroy dangerous Arrow Towers, and defeat each Guardian Chief to advance. Avoid unnecessary battles because army health carries between fights.

<b><color=#963F00>SAVE POWER FOR LATER</color></b>
Preserve healing, powerful relics, and Karaku Gold for the harder later floors. Checkpoint rewards are earned after every fourth completed floor.
```

### Source notes

- The primary guide was updated in January 2026 and documents locked lineups, persistent damage, checkpoints, relics, and buffs.
- Confidence: **High** for the core gameplay; **moderate** for floor-specific strategy because versions can differ.

Sources:

- [January 2026 Golden Kingdom guide](https://riseofkingdomsguides.com/golden-kingdom-event-guide-rok/)
- [January 2025 walkthrough](https://www.youtube.com/watch?v=MEy_VR1lgc4)

---

## Warpath

### Member instructions

Complete all troop-training quests every day:

- Train 2,000 Tier 2 or higher Infantry units.
- Train 2,000 Tier 2 or higher Archer units.
- Train 2,000 Tier 2 or higher Cavalry units.
- Train 2,000 Tier 2 or higher Siege units.
- Reach 10,000 total Tier 2 or higher trained units.
- Claim all completed rewards before 00:00 UTC.

All missions reset at 00:00 UTC. Unclaimed rewards and unfinished daily progress may be lost at reset.

### Leadership considerations

- Remind members that all four troop types are required for the individual milestones.
- Coordinate training with Mighty Army, Mightiest Governor, or other training events when their schedules overlap.
- Send a claim reminder before reset.

### Daily-mail summary

> Train 2,000 Tier 2+ units of every troop type and reach 10,000 total trained units today. Claim all completed rewards before the 00:00 UTC reset.

### Single-event announcement

**Mail subject:** `WARPATH`

```text
<size=34><b><color=#855400>WARPATH</color></b></size>
<color=#1E5F8A>[EVENT WINDOW — UTC]</color>

<size=27><b><color=#176B3A>TRAIN EVERY TROOP TYPE</color></b></size>
Complete all daily training objectives:
• 2,000 Tier 2+ Infantry
• 2,000 Tier 2+ Archers
• 2,000 Tier 2+ Cavalry
• 2,000 Tier 2+ Siege
• 10,000 total Tier 2+ units

<b><color=#963F00>CLAIM BEFORE RESET</color></b>
All missions reset daily at 00:00 UTC. Claim every completed reward before reset so your progress and rewards are not lost.
```

### Source notes

- The current community event reference was recrawled in July 2026 and lists the troop-type milestones, total target, and reset rule.
- Confidence: **High**, subject to the current in-game event page.

Source:

- [Warpath event reference](https://riseofkingdoms.fandom.com/wiki/Events/Warpath)
